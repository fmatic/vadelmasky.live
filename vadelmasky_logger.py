...
def load_store():
    ...
    if TODAY_FILE.exists():
        try:
            ...
            updated = store.get("summary", {}).get("updated", "")
...
def build_rows(aircraft_items, limit=50):
    rows = ""
...
def main():
    ...
    aircraft_list = live_aircraft_items(store)
...
<main>
    <div class="cards">
        <div class="card">
            <div class="value">{len(aircraft_list)}</div>
        ...
        <div class="card">
            <div class="value">{fmt_time(store["summary"]["updated"])}</div>
